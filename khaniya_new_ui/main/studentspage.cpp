...
void StudentsPage::onFilterChanged() {
    ...
    QList<UIStudent*> all = q.isEmpty() ? m_model->allStudents()
                                       : m_model->searchStudents(q);
    QList<UIStudent*> filtered;
    for (UIStudent *s : all) {
...
void StudentsPage::populateTable(QList<UIStudent*> Students) {
    m_table->setRowCount(Students.size());
    for (int i=0; i<Students.size(); i++) {
        UIStudent *s = Students[i];
