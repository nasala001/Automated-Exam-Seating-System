...
SeatCell& HallModel::seatAt(const QString &block, int sub, int bench, int seat) {
    ...
    if (bi < 0 || sub < 1 || sub > 3 || bench < 1 || bench > 6 || seat < 1 || seat > 2) {
        static SeatCell dummy;
        return dummy;
    }
...
bool HallModel::autoAssign() {
    ...
    std::sort(deptKeys.begin(), deptKeys.end(), [&](const QString& a, const QString& b) {
    ...
    for (int r = 0; r < m_rows; ++r) {
        ...
        int rowStartBucket = (buckets.size() > 1 && m_cols % buckets.size() == 0)
                           ? (r % buckets.size()) : currentBucket;
        currentBucket = rowStartBucket;

        for (int c = 0; c < m_cols; ++c) {
            SeatCell &cell = m_seatsGrid[r][c];
            if (cell.studentId != -1 || cell.locked) continue;

            // Find the next student from a different department than the left neighbor
            QString leftDept;
            if (c > 0 && m_seatsGrid[r][c-1].studentId != -1) {
                UIStudent* leftSt = findById(m_seatsGrid[r][c-1].studentId);
                if (leftSt) leftDept = leftSt->department;
            }
            ...
            for (int attempt = 0; attempt < buckets.size(); ++attempt) {
                ...
                if (indices[bIdx] < buckets[bIdx].size()) {
                    ...
                    if (m_activeRoomID == "MAIN_ROOM") {
                        ...
                        int sub = (c % 6) / 2 + 1;
                        int seat = (c % 2) + 1;
                        assignSeat(candidate->id, HallConst::BLOCK_NAMES[bi], sub, bench, seat);
                    } else {
            ...
            for (int b = 0; b < buckets.size(); ++b) {
                if (indices[b] < buckets[b].size()) { allPlaced = false; break; }
            }
            if (allPlaced) goto done;
        }
    }
done:
...
bool HallModel::lockSeat(int row, int col, bool lock) {
    if (row < 0 || row >= m_rows || col < 0 || col >= m_cols) return false;
    SeatCell &cell = m_seatsGrid[row][col];
    if (lock && cell.studentId != -1) return false;
    cell.locked = lock;
    emit dataChanged();
    return true;
}

void HallModel::setActiveRoom(const QString& hallID, const QString& roomID, int rows, int cols, const QString& venueName) {
    ...
    m_activeVenueCode = QString::fromStdString(normalizeVenueName(nameToNormalize.toStdString()));
...
int HallModel::occupiedCount() const {
    ...
    for (int r = 0; r < m_rows; ++r)
        for (int col = 0; col < m_cols; ++col)
            if (m_seatsGrid[r][col].studentId != -1) c++;
    return c;
}

int HallModel::occupiedInBlock(const QString &block) const {
    int bi = blockIndex(block);
    if (bi < 0 || m_activeRoomID != "MAIN_ROOM") return 0;
    int c = 0;
    int rStart = (bi / 3) * 6;
    int cStart = (bi % 3) * 6;
...
void HallModel::loadSampleData() {
    ...
    if (students.empty()) {
        qWarning() << "Could not load sample data: data/students.csv not found or empty.";
        return;
    }
...
void HallModel::saveToCSV(const QString &path) const {
    ...
    // Header
    ts << "name,registrationNo,rollNo,program,department,subject,semester,isPhysicallyImpaired\n";
...
void HallModel::loadFromCSV(const QString &path) {
    ...
    while (!ts.atEnd()) {
        ...
        s.registrationNo = fields[1].trimmed();
        s.rollNumber = fields[2].trimmed();
        s.program = fields[3].trimmed();
        s.department = fields[4].trimmed();
        s.subject = fields[5].trimmed();
        s.semester = fields[6].trimmed().toInt();
        s.isPhysicallyImpaired = (fields.size() > 7 && fields[7].trimmed().toLower() == "true");
        s.status = SeatStatus::Empty;
        m_students[s.id] = s;
    }
    emit dataChanged();
